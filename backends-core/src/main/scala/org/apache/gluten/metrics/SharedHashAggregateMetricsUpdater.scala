...
trait SharedHashAggregateMetricsUpdater extends MetricsUpdater {
  def updateAggregationMetrics(
      aggregationMetrics: java.util.ArrayList[OperatorMetrics],
      aggParams: AggregationParams): Unit
}

abstract class SharedHashAggregateMetricsUpdaterBase(val metrics: Map[String, SQLMetric])
  extends SharedHashAggregateMetricsUpdater {
  ...
  final override def updateAggregationMetrics(
      aggregationMetrics: java.util.ArrayList[OperatorMetrics],
      aggParams: AggregationParams): Unit = {
    var idx = 0
    ...
    updateAggSpecificMetrics(aggMetrics)
    idx += 1
  ...
  protected def updateAggSpecificMetrics(aggMetrics: OperatorMetrics): Unit = {}
}
