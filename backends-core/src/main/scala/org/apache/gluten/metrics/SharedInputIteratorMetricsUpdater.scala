...
abstract class SharedInputIteratorMetricsUpdater(
    val metrics: Map[String, SQLMetric],
    val forBroadcast: Boolean)
  extends MetricsUpdater {

  final override def updateNativeMetrics(opMetrics: IOperatorMetrics): Unit = {
    if (opMetrics != null) {
      ...
      if (!forBroadcast) {
        if (operatorMetrics.outputRows == 0 && operatorMetrics.outputVectors == 0) {
          // Sometimes, native backend does not update metrics for intermediate operator,
        ...
        updateExtraOutputMetrics(operatorMetrics)
      }
    }
  }

  protected def updateExtraOutputMetrics(operatorMetrics: OperatorMetrics): Unit = {}
}
