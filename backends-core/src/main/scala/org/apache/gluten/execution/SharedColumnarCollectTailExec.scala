...
/**
 * Shared collect-tail implementation for backends that can retain incoming columnar batches and
 * slice the first overflow batch using a backend-specific primitive.
 */
abstract class SharedColumnarCollectTailExec(limit: Int, child: SparkPlan)
  extends ColumnarCollectTailBaseExec(limit, child) {

  protected def sliceBatch(batch: ColumnarBatch, offset: Int, length: Int): ColumnarBatch
  ...
  override protected def collectTailRows(
    ...
    if (overflow > 0) {
      ...
      val sliced = sliceBatch(first, overflow.toInt, keep.toInt)
