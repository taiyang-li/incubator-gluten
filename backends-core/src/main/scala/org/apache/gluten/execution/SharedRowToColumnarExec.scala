...
/**
 * Backend-agnostic implementation of `Row -> ColumnarBatch` conversion. Backends only need to
 * subclass it and supply the per-backend preferred batch byte size and the broadcast unsafe
 * relation builder.
 */
abstract class SharedRowToColumnarExec(child: SparkPlan) extends RowToColumnarExecBase(child) {
