...
#ifndef GLUTEN_DYNAMIC_SCALAR_TYPE_DISPATCH
#error "GLUTEN_DYNAMIC_SCALAR_TYPE_DISPATCH must be defined before including VariantToVectorConverter.cc"
#endif

#ifndef GLUTEN_UNSUPPORTED
#error "GLUTEN_UNSUPPORTED must be defined before including VariantToVectorConverter.cc"
#endif
...
VectorPtr setVectorFromVariantsByKind<TypeKind::VARBINARY>(
  ...
  GLUTEN_UNSUPPORTED("Return of VARBINARY data is not supported");
}
...
VectorPtr setVectorFromVariants(const TypePtr& type, const std::vector<variant>& values, memory::MemoryPool* pool) {
  return GLUTEN_DYNAMIC_SCALAR_TYPE_DISPATCH(setVectorFromVariantsByKind, type->kind(), values, type, pool);
}
