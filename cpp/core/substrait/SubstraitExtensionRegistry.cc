...
template <typename T>
bool SubstraitExtensionRegistry::BiDirectionHashMap<T>::putIfAbsent(const int& key, const T& value) {
...
int SubstraitExtensionRegistry::getReferenceNumber(const ExtensionFunctionId& extensionFunctionId) {
  ...
  extensionFunctions_->putIfAbsent(functionReferenceNumber_, extensionFunctionId);
  return functionReferenceNumber_;
}

void SubstraitExtensionRegistry::addExtensionsToPlan(::substrait::Plan* plan) const {
